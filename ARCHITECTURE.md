...
- **New this iteration (cache-backed discovery feed + DEKHO cross-repo
  sync):** full detail in `docs/SEARCH_CACHE.md` — summarized here since
  it touches `App.tsx` state shape directly. The default Discovery view
  now opens on `browseCache()`/`browseCachedResults()` — a paginated
  browse of the *entire* local search-result cache, infinite-scrolled via
  an `IntersectionObserver` sentinel — instead of a live "trending"
  fetch. Selected filter chips and the typed search box both narrow this
  same feed locally (AND'd together, debounced while typing), with zero
  live fetch; an explicit search submit still hits YouTube, but renders
  in its own pinned, dismissible section above the feed rather than
  merging into it. Exposed at `GET /api/search-cache` — a real Vercel
  function (`api/search-cache.ts`), not a dev-only route, since
  `browseCache()` only ever reads the committed cache files that ship
  with the deployment; `vite.config.ts`'s dev middleware handles the same
  path locally with the same underlying call. (An earlier version of
  this feature merged live-search results onto the cache feed instead of
  separating them, and briefly shipped as `/api/dev/search-cache` —
  dev-only, and silently empty on Vercel as a result. Both fixed.)
  Renamed `popoutVideoId` → `requestedVideoId`, now dual-purpose: a
  popout still opens a separate window/tab as before, but a non-popout
  `requestedVideoId` triggers inline `runAnalysis()` playback — this is
  what lets `?discover=<query>&videoId=<id>` fire a search *and*
  inline-play a specific result from one URL. Sibling DEKHO project also
  writes real playlists here — `data/playlists/dekho-*.json`, one per
